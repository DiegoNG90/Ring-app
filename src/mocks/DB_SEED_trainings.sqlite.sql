...
SELECT 'Light spar', 'SPARRING_2', 'Rutina de sparring corta'
WHERE NOT EXISTS (
  SELECT 1 FROM trainings WHERE title = 'Light spar' AND training_type = 'SPARRING_2'
);

INSERT INTO training_rounds (training_id, round_number, duration_seconds, rest_seconds, repetitions)
SELECT t.id, 2, 120, 60, 0
FROM trainings t
WHERE t.title = 'Light spar' AND t.training_type = 'SPARRING_2'
  AND NOT EXISTS (
...
INSERT INTO trainings (title, training_type, description)
SELECT 'Medium spar', 'SPARRING_2', 'Rutina de sparring mediana (6 rounds)'
WHERE NOT EXISTS (
  SELECT 1 FROM trainings WHERE title = 'Medium spar' AND training_type = 'SPARRING_2'
);

INSERT INTO training_rounds (training_id, round_number, duration_seconds, rest_seconds, repetitions)
SELECT t.id, 6, 120, 60, 0
FROM trainings t
WHERE t.title = 'Medium spar' AND t.training_type = 'SPARRING_2'
  AND NOT EXISTS (
    SELECT 1 FROM training_rounds r WHERE r.training_id = t.id
  );
