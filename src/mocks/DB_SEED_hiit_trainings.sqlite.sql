...
SELECT 'Functional 3 laps light', 'HIIT funcional, 3 vueltas'
WHERE NOT EXISTS (
  SELECT 1 FROM trainings WHERE title = 'Functional 3 laps light'
);
...
INSERT INTO trainings (title, description)
SELECT 'Functional 4 laps light', 'HIIT funcional, 4 vueltas'
WHERE NOT EXISTS (
  SELECT 1 FROM trainings WHERE title = 'Functional 4 laps light'
);
